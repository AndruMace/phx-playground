defmodule NewWeb.UserAuthTest do
  use NewWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias New.Accounts
  alias New.Accounts.Scope
  alias NewWeb.UserAuth

  import New.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, NewWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      NewWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        NewWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
